...
public class IntegrationTests {
	...
	void shouldFindVetAndVerifySpecialty() {
		...
		Set<String> actualSpecialties = vet.getSpecialties()
			.stream()
			.map(Specialty::toString)
			.collect(Collectors.toSet());
		Set<String> expectedSpecialties = specialties.stream().map(Specialty::toString).collect(Collectors.toSet());
	...
	void shouldAddNewVisitForPet() {
		...
		assertThat(visits2).hasSize(found + 1).allMatch(value -> value.getId() != null);
	}
