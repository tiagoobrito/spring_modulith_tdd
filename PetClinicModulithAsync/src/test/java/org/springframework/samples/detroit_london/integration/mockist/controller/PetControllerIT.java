...
class PetControllerIT {
	...
	void post_newPet_success_calls_save_with_ownerId_and_redirects() throws Exception {
		mvc.perform(post("/owners/6/pets/new").param("name", "Buddy")
			.param("birthDate", "2019-05-20")
			.param("type.id", "2")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/owners/6"));

		verify(serviceSpy, atLeastOnce()).findPetTypes(); // @ModelAttribute
		verify(serviceSpy).save(argThat(p -> "Buddy".equals(p.getName())
				&& LocalDate.of(2019, 5, 20).equals(p.getBirthDate()) && Integer.valueOf(6).equals(p.getOwner_id())
				&& p.getType() != null && Integer.valueOf(2).equals(p.getType().getId())));
		verifyNoMoreInteractions(serviceSpy);
	}
	...
	void post_editPet_success_calls_save_with_existing_id_and_redirects() throws Exception {
		mvc.perform(post("/owners/6/pets/7/edit").param("id", "7")
			...
			.param("type.id", "1")).andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/owners/6"));

		verify(serviceSpy, atLeastOnce()).findPetTypes();
		verify(serviceSpy, atLeastOnce()).getPetById(7);
		verify(serviceSpy, atLeastOnce())
			.save(argThat(p -> Integer.valueOf(7).equals(p.getId()) && Integer.valueOf(6).equals(p.getOwner_id())
					&& "Samantha edited".equals(p.getName()) && LocalDate.of(2012, 9, 4).equals(p.getBirthDate())
					&& p.getType() != null && Integer.valueOf(1).equals(p.getType().getId())));
		verifyNoMoreInteractions(serviceSpy);
	}
