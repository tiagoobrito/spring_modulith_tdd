...
public class VisitRepositoryImpl implements VisitRepository {
	...
	public Set<Visit> findVisitByPetId(Integer id) {
		return jdbcClient.sql("SELECT * FROM visits WHERE pet_id = ?").param(id).query(Visit.class).set();
	}

	@Override
	public List<Visit> findAll() {
		return jdbcClient.sql("SELECT * FROM visits").query(Visit.class).list();
	}

	@Override
	public Visit findById(Integer id) {
		return jdbcClient.sql("SELECT * FROM visits WHERE id = ?").param(id).query(Visit.class).optional().orElse(null);
	}
